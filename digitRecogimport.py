...
def load_digits_custom(img_file):
    ...
    for index,rect in enumerate(digits_rectangles):
        ...
        if index>0 and (index+1) % 10 == 0:
            start_class += 1
    cv2.imwrite("/images/training_box_overlay.png",im)
...
def predict(model, TEST_USER_IMG):
    return_string = proc_user_img(TEST_USER_IMG, model)
    #print(return_string)  
...
def train():   
    TRAIN_MNIST_IMG = 'images/digits.png' 
    TRAIN_USER_IMG = 'images/custom_train_digits.jpg'
    #TEST_USER_IMG = 'numbertemp.jpg'
    ...
    X_train, X_test, y_train, y_test = train_test_split(train_digits_data, labels, test_size=0.33, random_state=50)
...
'''
imagestring = 'images/temp.jpg'
if(counter is 0):
    number_model = train()
predict(number_model, imagestring)
counter = counter+1
'''
